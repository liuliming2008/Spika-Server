ChatRoom = function(optDebug) {
    ...
    var api = {
      ...
      , regist:function(email,token,teamdomain,callback) {
          sess.call('regist', {email:email, token:token, teamdomain:teamdomain}).then(function(args) {
        	  callback(args);
          }, function(args) {
        	  callback(args);
          });
      ...
      ,call:function(command,params,callback_success,callback_fail){
    	  sess.call(command, params).then(function(args) {
    		  callback_success(true,jQuery.parseJSON(args.data),params); 
          }, function(args) {
        	  	if( callback_fail )
